...
- قواعد Firestore هي مصدر الحماية الحقيقي، وليس إخفاء زر لوحة الأدمن في الواجهة.
- حساب الأدمن يتم التحقق منه داخل `firestore.rules` باستخدام بريد Google الموثق.
- الطالب يستطيع قراءة ملفه الشخصي فقط، ولا يستطيع قراءة بيانات باقي الطلاب.
- الطالب لا يستطيع تغيير `isAdmin` أو `paidMonths` أو `email` أو `createdAt`.
- سجلات الحضور الخاصة بالطالب يمكنه قراءتها فقط، بينما الكتابة متاحة للأدمن فقط.
- `ADMIN_EMAILS` في `src/lib/firebase.js` تستخدم لتحديد ظهور واجهة الأدمن فقط؛ لا تعتمد عليها كطبقة أمان.
- إعدادات Firebase Web مثل `apiKey` ليست كلمة مرور أو Service Account secret. لا ترفع أبدًا ملفات Service Account أو مفاتيح Admin SDK أو ملفات `.env` التي تحتوي أسرارًا.
