...
// الاعتذارات خاصة بقادة الفريق بس (مش الأدمن العام)
export default function AdminExcuses() {
  ...
  useEffect(() => {
    ...
    const applyUpdate = (docs) => {
      docs.forEach((d) => { combined[d.id] = { id: d.id, ...d.data() }; });
    ...
    adminTeamIds.forEach((teamId) => {
      ...
      unsubs.push(onSnapshot(q, (snap) => applyUpdate(snap.docs)));
    });
  ...
  if (adminTeamIds.length === 0 && adminSchoolIds.length === 0) {
    return <p className="text-sm font-body text-ink/40">الاعتذارات متاحة بس لقادة الفرق/المدارس.</p>;
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {excuses.length === 0 && <p className="text-sm font-body text-ink/40">لا توجد اعتذارات حاليًا.</p>}
    </div>
  );
}
