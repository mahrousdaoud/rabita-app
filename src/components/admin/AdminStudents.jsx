...
export default function AdminStudents() {
  ...
  useEffect(()=>{
    const unsubs=[], combined={}; const flush=()=>setStudents(Object.values(combined).sort((a,b)=>(a.name||"").localeCompare(b.name||"","ar")));
    const add=s=>{s.docs.forEach(d=>combined[d.id]={id:d.id,...d.data()});flush()};
    if(isSuperAdmin) unsubs.push(onSnapshot(query(collection(db,"students"),orderBy("name","asc")),add));
    else {adminTeamIds.forEach(id=>unsubs.push(onSnapshot(query(collection(db,"students"),where("teamId","==",id)),add))); adminSchoolIds.forEach(id=>unsubs.push(onSnapshot(query(collection(db,"students"),where("schoolId","==",id)),add)));}
  ...
  const allowedTeams=isSuperAdmin?teams:teams.filter(t=>adminTeamIds.includes(t.id));
  return <div>
    <div className="flex flex-wrap gap-3 mb-4"><input placeholder="دور بالاسم أو الفريق أو المنطقة..." value={search} onChange={e=>setSearch(e.target.value)} className="field-input flex-1 min-w-[220px]"/>
